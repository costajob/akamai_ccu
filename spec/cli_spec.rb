...
describe AkamaiCCU::CLI do
  ...
  it "must call invalidate on staging by cp code" do
    cli = AkamaiCCU::CLI.new(args: ["--cp=#{Stubs.cpcodes.join(",")}", "--secret=#{Stubs.txt_path}"], action: "invalidate", io: io, wrapper_klass: Stubs::Wrapper, secret_klass: Stubs::Secret, endpoint_klass: Stubs::Endpoint)
    cli.call
  ...
  it "must call delete on production by cp url" do
    cli = AkamaiCCU::CLI.new(args: ["--url=#{Stubs.urls.join(",")}", "--secret=#{Stubs.edgerc_path}", "--production"], action: "delete", io: io, wrapper_klass: Stubs::Wrapper, secret_klass: Stubs::Secret, endpoint_klass: Stubs::Endpoint)
    cli.call
  ...
  it "must call invalidate on production by bulk urls" do
    cli = AkamaiCCU::CLI.new(args: ["--bulk=#{Stubs.bulk_urls.path}", "--secret=#{Stubs.edgerc_path}", "--production"], action: "invalidate", io: io, wrapper_klass: Stubs::Wrapper, secret_klass: Stubs::Secret, endpoint_klass: Stubs::Endpoint)
    cli.call
  ...
  it "must call delete on staging by bulk cpcodes" do
    cli = AkamaiCCU::CLI.new(args: ["--bulk=#{Stubs.bulk_codes.path}", "--secret=#{Stubs.edgerc_path}"], action: "delete", io: io, wrapper_klass: Stubs::Wrapper, secret_klass: Stubs::Secret, endpoint_klass: Stubs::Endpoint)
    cli.call
  ...
  it "must consider only CP codes when mixing entries in bulk file" do
    cli = AkamaiCCU::CLI.new(args: ["--bulk=#{Stubs.bulk_mixed.path}", "--secret=#{Stubs.edgerc_path}"], action: "invalidate", io: io, wrapper_klass: Stubs::Wrapper, secret_klass: Stubs::Secret, endpoint_klass: Stubs::Endpoint)
    cli.call
  ...
  it "must warn of no contents when entries in bulk file are invalid" do
    cli = AkamaiCCU::CLI.new(args: ["--bulk=#{Stubs.bulk_invalid.path}", "--secret=#{Stubs.edgerc_path}"], action: "invalidate", io: io, wrapper_klass: Stubs::Wrapper, secret_klass: Stubs::Secret, endpoint_klass: Stubs::Endpoint)
    cli.call
    Stubs.strip_log(io.string).must_equal "WARN -- : specify contents to purge by bulk, CP codes or urls"
  end

  it "must give precedence to cp option if also url one is specified" do
    cli = AkamaiCCU::CLI.new(args: ["--url=#{Stubs.urls.join(",")}", "--cp=#{Stubs.cpcodes.join(",")}", "--secret=#{Stubs.edgerc_path}"], action: "invalidate", io: io, wrapper_klass: Stubs::Wrapper, secret_klass: Stubs::Secret, endpoint_klass: Stubs::Endpoint)
    cli.call
    Stubs.strip_log(io.string).must_equal "INFO -- : secret=.edgerc;endpoint=ccu/v3/invalidate/cpcode/staging;objects=12345,98765"
  end

  it "must give precedence to bulk if also url option is specified" do
    cli = AkamaiCCU::CLI.new(args: ["--url=#{Stubs.urls.join(",")}", "--bulk=#{Stubs.bulk_codes.path}", "--secret=#{Stubs.edgerc_path}"], action: "invalidate", io: io, wrapper_klass: Stubs::Wrapper, secret_klass: Stubs::Secret, endpoint_klass: Stubs::Endpoint)
    cli.call
    Stubs.strip_log(io.string).must_equal "INFO -- : secret=.edgerc;endpoint=ccu/v3/invalidate/cpcode/staging;objects=12345,98765"
  end

  it "must give precedence to bulk if also cpcode option is specified" do
    cli = AkamaiCCU::CLI.new(args: ["--cp=#{Stubs.cpcodes.join(",")}", "--bulk=#{Stubs.bulk_urls.path}", "--secret=#{Stubs.edgerc_path}"], action: "invalidate", io: io, wrapper_klass: Stubs::Wrapper, secret_klass: Stubs::Secret, endpoint_klass: Stubs::Endpoint)
    cli.call
  ...
  it "must accept headers to sign" do
    cli = AkamaiCCU::CLI.new(args: ["--cp=#{Stubs.cpcodes.join(",")}", "--secret=#{Stubs.txt_path}", "--headers=Accept,Content-Length"], action: "invalidate", io: io, wrapper_klass: Stubs::Wrapper, secret_klass: Stubs::Secret, endpoint_klass: Stubs::Endpoint)
    cli.call
