...
describe AkamaiCCU::Response do
  let(:error) { AkamaiCCU::Response.new(body: Stubs.error_body) }
    let(:ack) { AkamaiCCU::Response.new(body: Stubs.ack_body, time: Time.new(2017,6,20,12,19,11, "+01:00")) }

  it "must accept a plain Hash as the body" do
    response = AkamaiCCU::Response.new(body: { "status"=>201 })
    response.successful?.must_equal true
  end

  it "must accept a JSON string as the body" do
    response = AkamaiCCU::Response.new(body: { "status"=>201 }.to_json)
    response.successful?.must_equal true
  end

  it "must represent error as a string" do
    error = AkamaiCCU::Response.new(body: Stubs.error_body)
  ...
  it "must represent ack as a string" do
    ack = AkamaiCCU::Response.new(body: Stubs.ack_body, time: Time.new(2017,6,20,12,19,11, "+01:00"))
